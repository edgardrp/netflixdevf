...
function upgradeSubscription(_, args, context, info) {
    ...
    return Subscription.findById(subscription_id).then(subscription => {
        if (subscription.type_subscription == args.type) {
            throw new Error("You can't update to the same subscription!")
        }
...
function addSource(_, args, context, info) {
    ...
    stripe.customers.createSource(user_payment, { source: args.source }, (err, customer) => {
        if (err) {
            throw err;
        }
    })
